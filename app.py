...
with st.container():
    st.markdown('<div id="upload-section" class="snap-anchor"></div>', unsafe_allow_html=True)
    st.markdown(
        ...
        <script>
          (() => {
            ...
            const sectionClasses = {
              "hero-section": "hero-snap-section",
              "upload-section": "landing-panel upload-panel",
              "overview-section": "workflow-section overview-section upload-focus-compact",
              "workspace-section": "workspace-section",
              "ai-analysis-section": "ai-snap-section",
              "communications-section": "communications-section",
              "ai-summary-section": "ai-summary-section"
            };

            const getMainBlock = () =>
              doc.querySelector('.block-container > div[data-testid="stVerticalBlock"]') ||
              doc.querySelector('.block-container [data-testid="stVerticalBlock"]');

            const sectionShellFor = (marker, mainBlock) => {
              if (!marker) return null;
              const directElement = marker.closest('[data-testid="stElementContainer"]') || marker.parentElement;
              if (marker.id === "hero-section") return directElement;

              let node = marker.parentElement;
              while (node && node !== mainBlock) {
                if (node.getAttribute?.("data-testid") === "stVerticalBlock") {
                  return node.closest('[data-testid="stElementContainer"]') || directElement;
                }
            ...
            const classifySections = () => {
              ...
              Object.entries(sectionClasses).forEach(([id, classNames]) => {
                const marker = doc.getElementById(id);
                const shell = sectionShellFor(marker, mainBlock);
                if (!marker || !shell || shell.dataset.aisiSectionId === id) return;
                shell.classList.add(...classNames.split(" "));
                shell.dataset.aisiSectionId = id;
              });
            };

            window.parent.scrollToSection = (id) => {
            ...
            classifySections();
            window.parent.__aisiSimpleNavObserver?.disconnect();
            window.parent.__aisiSimpleNavObserver = new MutationObserver(classifySections);
            window.parent.__aisiSimpleNavObserver.observe(getMainBlock() || doc.body, { childList: true, subtree: true });
          })();
        </script>
        """,
        height=0,
...
with st.container():
    st.markdown('<div id="overview-section" class="snap-anchor"></div>', unsafe_allow_html=True)
    ...
    upload_scroll_key = f"{data_signature}:{st.session_state.get('upload_scroll_nonce', 0)}"
    components.html(
        f"""
        <script>
          (() => {{
            const key = {upload_scroll_key!r};
            const run = () => {{
              if (window.parent.__aisiUploadSectionScroll === key) return;
              window.parent.__aisiUploadSectionScroll = key;
              window.parent.scrollToSection?.("overview-section");
            }};
            window.parent.requestAnimationFrame(() => {{
              window.parent.requestAnimationFrame(run);
            }});
          }})();
        </script>
        """,
...
with st.container():
    st.markdown('<div id="workspace-section" class="snap-anchor"></div>', unsafe_allow_html=True)
...
with st.container():
    st.markdown('<div id="ai-analysis-section" class="snap-anchor"></div>', unsafe_allow_html=True)
    ...
    with ai_action_col:
        st.markdown(
            f'<section class="ai-analysis-panel">{load_template("cta_ai_box.html")}</section>',
            unsafe_allow_html=True,
        )
...
if generate_ai:
        ...
        components.html(
            ...
            <script>
                setTimeout(() => {{
                    ...
                    if (window.parent.__aisiAiLoadingScroll !== key) {{
                        window.parent.__aisiAiLoadingScroll = key;
                        window.parent.scrollToSection?.("ai-analysis-section");
                    }}
                }}, 250);
            </script>
            """,
    ...
    try:
        logger.info("Starting Gemini analysis for %s.", data_signature)
        ai_result = generate_ai_decision_layer(enriched_df)
        ai_source = AI_SOURCE_GEMINI
        ai_mode = AI_MODE_LABELS[ai_source]
    except AIQuotaExceededError:
        logger.warning("Gemini failed | category=quota | action=trying_hugging_face")
        try:
            ...
            logger.warning(
                "Hugging Face failed after Gemini quota | category=%s | action=using_backend_fallback",
                getattr(hf_error, "category", "unknown"),
            )
        ...
        logger.warning(
            "Gemini failed | category=non_quota | detail=%s | action=trying_hugging_face",
            str(error)[:500],
        )
        try:
            ai_result = generate_huggingface_decision_layer(enriched_df)
            ...
            logger.warning(
                "Hugging Face failed after Gemini error | category=%s | action=using_backend_fallback",
                getattr(hf_error, "category", "unknown"),
            )
            ai_result = build_fallback_ai_decision(enriched_df)
            ai_source = AI_SOURCE_BACKEND_FALLBACK
...
if ai_result:
    ...
    components.html(
        ...
        <script>
            setTimeout(() => {{
                ...
                if (window.parent.__aisiAiResultScroll !== key) {{
                    window.parent.__aisiAiResultScroll = key;
                    window.parent.scrollToSection?.("communications-section");
                }}
            }}, 700);
        </script>
        """,
    ...
    with st.container():
        st.markdown('<div id="communications-section" class="snap-anchor"></div>', unsafe_allow_html=True)
        ...
        if banner_status and banner_message:
            banner_html = (
                load_template("ai_status_banner.html")
                .replace("{{status}}", escape_html(banner_status))
                .replace("{{ status }}", escape_html(banner_status))
                .replace("{{message}}", escape_html(banner_message))
                .replace("{{ message }}", escape_html(banner_message))
            )

        st.markdown(
            f"""
            <div class="ai-status-brief-row">
                <div class="ai-status-brief-banner">{banner_html}</div>
                <a class="pill ai-brief-link" href="#ai-decision-brief">{escape_html(decision_brief_label)}</a>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with st.container():
        ...
        st.markdown(
            f'<div id="ai-decision-brief" class="section-title">7. {decision_brief_label}</div>',
            unsafe_allow_html=True,
        )
        render_template(
            "insight_box.html",
